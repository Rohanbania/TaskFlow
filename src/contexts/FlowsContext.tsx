...
interface FlowsContextType {
  ...
  addTask: (flowId: string, taskTitle: string, taskDescription: string, date?: string, startTime?: string, endTime?: string) => void;
...
export function FlowsProvider({ children }: { children: ReactNode }) {
  ...
  const addTask = useCallback((flowId: string, taskTitle: string, taskDescription: string, date?: string, startTime?: string, endTime?: string) => {
    const newTask: Task = {
      id: crypto.randomUUID(),
      title: taskTitle,
